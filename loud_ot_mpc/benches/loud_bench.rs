...
fn wrk17_protocol<FE: FF, Sender: SVoleSender<Msg = FE>, Receiver: SVoleReceiver<Msg = FE>>() {
...
fn hss17_protocol<FE: FF, Sender: SVoleSender<Msg = FE>, Receiver: SVoleReceiver<Msg = FE>>() {
    let (sender, receiver) = UnixStream::pair().unwrap();
    let handle = std::thread::spawn(move || {
        let mut rng = AesRng::new();
        let reader = BufReader::new(sender.try_clone().unwrap());
...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("loud_ot whole protocol", |b| b.iter(|| wrk17_protocol::<F128b, Sender<F128b>, Receiver<F128b>>()));
}
